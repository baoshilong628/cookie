...
const {debounce, isArray} = require("lodash");
...
const getParamsFromParseResult = result => {
    const firstParamResult = result.find(({ type }) => type === 'param');
    if (!firstParamResult) {
        return [];
    }
    if (!isArray(firstParamResult.value)) {
        return [];
    }
    return firstParamResult.value.map(({ key, value, isRequired }) => {
